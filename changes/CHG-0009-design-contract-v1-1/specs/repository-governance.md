...
#### Requirement
从 CHG-0009 起，每个 Change 必须声明 `design_machine_contract_version: 1` 并维护 `design.yaml`，其引用、facet、事实、假设、开放问题、定义和追踪必须可确定性校验。
#### Scenario SCN-GOV-V11-001-01 生成机器契约
- Given: 使用 `tools/new_change.py` 创建 CHG-0009 及以后的新 Change
- When: 生成器完成初始化
...
- Given: CHG-0001 至 CHG-0008 不使用机器契约
- When: 执行统一检查
...
#### Requirement
`design.yaml` 中的 Definition 与 Traceability 必须引用真实 Spec/Test，且 Definition/Traceability 中的每个 Test 必须在 `tests.yaml` 中覆盖对应 Spec。
