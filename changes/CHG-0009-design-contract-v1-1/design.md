...
公共 Seam 为 `python tools/validate_design_machine.py`、`python tools/context.py CHG-0009 --bundle` 和 `python tools/check.py` 的退出码与结构化输出。合法仓库返回0，反例返回非0并包含稳定错误关键词。
...
可从 `tools/check.py` 移除机器校验入口并恢复模板、生成器、Context Pack和Skill；CHG-0001至CHG-0008不需要迁移或回写。

## 迁移方案

MIG-GOV-V11-001：CHG-0001至CHG-0008继续使用Design Contract v1；CHG-0009及以后同时声明 `design_contract_version: 1` 和 `design_machine_contract_version: 1`。v1 Markdown校验继续作为兼容层，正式引用与追踪逐步迁移到 `design.yaml`。
